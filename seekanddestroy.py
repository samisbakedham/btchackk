#!/usr/bin/python
#sws 10/04/2021 github.com/samisbakedham
'''
		...
		with open(filename) as f:
			for line in f:
				if pub in line:
					...
					server.ehlo()
					server.starttls()
					server.login("samuelsafahi@icloud.com", "ldni-cxze-mvfq-xmwo")
					fromaddr = "samuelsafahi@icloud.com"
					toaddr = "samuelsafahi@icloud.com"
					...
					with open('Wallets.txt','a') as f:
						...
						f.close()
					time.sleep(30)
					print ('Yay! ---- ' +dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), pub, priv)
					break
